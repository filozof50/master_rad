...
Na slici \ref{fig:poredjenje_instrukcija} se mogu videti rezultati poređenja pokrivenosti naredbi (u procentima), dok slika \ref{fig:poredjenje_grana} prikazuje rezultate poređenja pokrivenosti grana (u procentima). Rezultati pokazju da je u najvećem broju slučajeva algoritam koji predlažu autori alata bolji što se tiče pokrivenosti koda (najviše za 7.48\%), ali postoje i slučajevi gde je algoritam BFS-DFS dao bolje rezultate (naredbe \textit{echo}, \textit{false}, \textit{kill}, \textit{nice}, \textit{pwd} i \textit{true}).
