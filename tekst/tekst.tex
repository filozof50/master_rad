...
\begin{description}
    ...
    \item \textbf{Knight Capital Group} - kompanija je 2012 godine izgubila 460 miliona dolara za samo 45 minuta. Ova Američka kompanija se bavila finansijama i radom na berzi. 2012 je postojala izmena sistema i refaktorisanje koda istog. Kada je unapređen softver, trebalo je da bude prekopiran na svih 8 servera koje je posedovala ova kompanija. Međutim, inženjer koji je trebalo to da uradi je zaboravio da na jedan server prekopira novu verziju koda. Kompanija nije imala drugog inženjera koji bi proverio da li je sve u redu, niti softver koji je mogao da otkrije da je došlo do greške.
    ...
    \item \textbf{Dhahran raketa} - raketa Saudijske Arabije je 1991. godine ubila 28 američkih vojnika i povredila još 100 ljudi u gradu Dahran (eng. \textit{Dhahran}) u Zalivskom ratu. Problem je bio vreme odziva sistema. Prilikom računanja vremena odziva od trenutka paljenja odbrambenog sistema je decimalni broj bio ,,odsečen'' na 24. decimali. Množenjem sa velikim brojem se ova greška gomila i prestaje da bude zanemariva. Greška u softveru je dovela do toga da se odbrambeni sistem ne aktivira na vreme i došlo je do gubitaka ljudskih života.
...
\subsubsection{Vrste testiranja}
U ovom delu je dat pregled najčešće korišćenih vrsta testiranja u razvoju softvera.

\paragraph{Testiranje jedinica koda} - podrazumeva testiranje manjih delova koda koji mogu da funkcionišu nezavisno. To može biti procedura, funkcija, klasa i slično. Ovaj vid testiranja se sprovodi konstantno i testove piše sam programer. Cilj jediničnih testova je da se omogući da i najsitnije celine koda imaju očekivanu funkcionalnost. Međusobna interakcija ovih jedinica se vrši drugim vrstama testiranja. U današnje vreme većina razvojnih okruženja ima podršku za pisanje jediničnih testova.

\paragraph{Komponentno testiranje} - slično kao i testiranje jedinica koda. Razlika je u tome što komponente predstavljaju nešto veće celine. Jedna komponenta uglavnom predstavlja nekoliko jedinica koda koje su povezane u celinu. Komponente se testiraju slično kao i jedinice koda, odvojeno od ostatka sistema. Testiranje komponente se vrši odmah nakon njenog nastanka, što znači da se i ovaj vid testiranja vrši konstatno tokom razvoja softvera. Ove vrste testova mogu da sprovode i programeri i testeri.

\paragraph{Integraciono testiranje} - testiranje većeg broja povezanih komponenti. Ovim testiranjem se proverava da li komponente komuniciraju na način na koji je to opisano u projektnoj dokumentaciji. Integraciono testiranje je na nešto višem nivou u odnosu na prethodna dva, jer proverava komunikaciju i interakciju između većeg broja komponenti, dok prethodno navedene vrste testiranja vrše izolaciju nekih funkcionalnosti sistema. Ove testove sprovodi tester.

\paragraph{Sistemsko testiranje} - testiranje sistema kao celine. Provera da li sistem ispunjava sve predviđene zahteve i da li nema grešaka u radu sistema. U ovu vrstu testiranja se često uključuju i sledeće podvrste:

\begin{description}
    \item \textbf{Istraživačko testiranje} - ovom vrstom testiranja se proveravaju alternativni pravci upotrebe sistema. Pored osnovne funkcije koja je tražena projektnom specifikacijom, softver možda može biti korišćen i na drugačiji način. Ovom vrstom testiranja testeri upravo to proveravaju. Istraživačko testiranje se uglavnom koristi na kraju, kada je softver već gotov proizvod i kada je funkcionalna celina. 
    
    \item \textbf{Testovi prihvatljivosti} - testovi koje sprovodi klijent uz pomoć razvnojnog tima. Razvojni tim ima ulogu da razjasni neke tehničke detalje sistema ukoliko za tim ima potrebe. Klijent uglavnom proverava da li proizvod ispunjava njegove potrebe i očekivanja.
\end{description}

\paragraph{Regresiono testiranje} - testiranje koje se sprovodi nakon što dođe do nekih izmena u sistemu. Ovo testiranje je neophodno da bi se utvrdilo da neka izmena nije dovela do grešaka u nekim drugim delovima sistema koji nisu bili obuhvaćeni zahtevanom izmenom.

\subsubsection{Tehnike testiranja}
U ovom delu je dat pregled najčešće korišćenih tehnika testiranja u razvoju softvera.

\paragraph{Testiranje metodom crne kutije} - poznato i kao \textit{testiranje ponašanja} (eng. \textit{Behavioral Testing}) je tehnika testiranja u kojoj interna implementacija i k\^od softvera nisu poznati testru. Kako tester ne zna za internu strukutru sistema za njega sistem predstavlja \textit{crnu kutiju} (eng. \textit{black box}), otuda potiče i naziv ove tehnike testiranja. Ovo tehnikom se uglavnom mogu otkriti sledeće greške:

\begin{itemize}
    \item Neispravne funkcionalnosti softvera.
    
    \item Greške u korisničkom interfejsu.
    
    \item Problemi s performansama.

\end{itemize}

Uglavnom se koristi u višim nivoima testiranja, recimo u sistemskom i integracionom testiranju.
Neke od metoda za kreiranje testova ovom tehnikom su:

\begin{description}
    \item \textbf{Testiranje pomoću klasa ekvivalencije} - gde se podaci grupišu u neke skupove pri čemu podaci iz istog skupa treba da proizvedu isti rezultat. Očekivano je da ako jedan test slučaj iz neke klase ekvivalencije otkrije grešku, i \textit{\textbf{svi}} drugi test slučajevi iz iste klase ekvivalencije bi trebalo da otkriju istu grešku. Takođe, ako neki test slučaj ne otkriva grešku, \textit{\textbf{nijedan}} test slučaj iz iste klase ekvivalencije ne bi trebalo da otkrije grešku.
    
    \item \textbf{Testiranje graničnih vrednosti} - metod kod koga se podaci takođe dele u klase ekvivalencije. Zatim se bira jedan granični slučaj iz klase ekvivalencije, jedan slučaj iznad graničnog i jedna slučaj ispod graničnog. Nad njima se vrši testiranje. Zavisno od prirode podataka pojmovi \textit{iznad} i \textit{ispod} su relativni. Intuicija iza ove tehnike je da je čest slučaj da se greške dešavaju baš prilikom graničnih slučajeva, recimo korišćenje $>$ umesto $\geq$.
    
    \item \textbf{Dijagrami stanja} - tehnika testiranja metodom crne kutije u kojoj se kreiraju dijagrami koji opisuju na koji način sistem reaguje zavisno od spoljnih događaja. Koristi se uglavnom za sisteme koji zavise od nekih akcija koje su se već desile ili zavise od nekih spoljinih faktora. Dijagrami prikazuju stanja kroz koja sistem prolazi i koji su to događaji koji navode sistem na promenu stanja. Takođe se opisuju i akcije koje dovode do promene stanja (zavisno od događaja koji se desio, sistem preduzima  neku akciju i menja stanje).
\end{description}

Glavna prednost ove metode testiranja je što se testiranje vrši iz ugla korisnika, i što je testove moguće osmisliti čim je poznata specifikacija softvera. S druge strane, specifikacija se jako često menja i nije jasno definisana što ovaj vid testiranja čini nešto komplikovanijim. Takođe, nije moguće istražiti sve delove sistema ako nije poznata njegova interna implementacija.

\paragraph{Testiranje metodom bele kutije} - je tehnika testiranja u kojoj su interna implementacija i k\^od softvera poznati onome ko vrši tesitranje. Ovu tehniku testiranja koriste i testeri ali programeri. Kako je poznat k\^od softvera, bira se putanja u kodu kojom treba da se prođe kako bi ona bila testirana. Na osnovu toga se dalje biraju ulazni podaci, i proverava se da li će izvršavanje programa dati očekivan rezultat. Pošto je k\^od poznat i moguće ga je videti, softver za onoga ko testira predstavlja \textit{belu kutiju} (eng. \textit{white box}), pa otuda i potiče naziv ove tehnike.

Ova tehnika se češće upotrebljava na nižim nivoima testiranja (testiranje jedinice koda i integraciono testiranje), ali može se koristiti i prilikom sistemskog testiranja. 

Idealno bi bilo kada bi bilo moguće ispitati sve moguće putanje kroz program. U praksi, zbog veličine i kompleksnosti softvera to nije moguće pa se teži da se maksimizuje pokrivenost koda:

\begin{itemize}
    \item Pokrivenost putanja - procenat putanja koje su izvršene barem jednom.
    
    \item Pokrivenost naredbi - procenat naredbi koje su izvršene barem jednom.
    
    \item Pokrivenost grana - procenat grana koje su izvršene barem jednom.
    
    \item Pokrivenost uslova - procenat uslova koji su izvršeni barem jednom.

    \item Pokrivenost funckija - broj funkcija koje su pozvane barem jednom.

\end{itemize}

Prednosti ove tehnike su što se testiranje vrši tokom celog ciklusa razvoja softvera i što se zbog poznavanja implementacije može lakše testirati ponašanje koje korisniku nije vidljivo već predstavlja interni rad programa. Glavna mana je što je potrebno generisati dobre testove kako bi se maksimizovala pokrivenost koda, i potrebni su stručni ljudi koji razumeju principe razvoja softvera, njegove specifičnosti i specifičnosti jezika u kome se softver implementira kako bi testiranje bilo dovoljno kvalitetno.
