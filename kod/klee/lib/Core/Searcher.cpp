...
BFSDFSSearcher::BFSDFSSearcher(double memoryPart, int instructionsSinceCoveredNew, Executor &executor)
    : exe(executor)
{
    runningBFS = true;
    mMemoryPart = memoryPart;
    instsSinceCoveredNew = instructionsSinceCoveredNew;
}
                         ...
                         const std::vector<ExecutionState *> &removedStates)
{
  ...
  for (std::vector<ExecutionState *>::const_iterator it = removedStates.begin(), ie = removedStates.end(); it != ie; ++it) {
    ...
    if (es == DFSStates.back()) {
      DFSStates.pop_back();
          ...
          if (!ok) {
              for (std::vector<ExecutionState*>::iterator it = BFSStates.begin(),
 ...
 if (stats::instructions >= instsSinceCoveredNew + 10000) {
