...
void Executor::stepInstruction(ExecutionState &state) {
  ...
  if (statsTracker) {
//      std::cout << "statsTracker->stepInstruction(state)\n";
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
  ...
  case Instruction::Br: {
    ...
    if (bi->isUnconditional()) {
      ...
      if (branches.first) {
//        std::cerr << "Branch first " << ki->info->line << "\n";
        transferToBasicBlock(bi->getSuccessor(0), bi->getParent(), *branches.first);
      }
      if (branches.second) {
//        std::cerr << "Branch second " << ki->info->line << "\n";
...
//  if (dynamic_cast<NewSearcher *>(searcher)->empty()) {
//      haltExecution = true;
//  }
}
...
void Executor::checkMemoryUsage() {
  if (stats::instructions % 500 == 0) {
    if (dynamic_cast<const MySearcher*>(searcher) != nullptr) {
        if (dynamic_cast<const MySearcher*>(searcher)->is_running_BFS()) {
            unsigned long mbs = (util::GetTotalMallocUsage() >> 20) + (memory->getUsedDeterministicSize() >> 20);

            if (mbs >= (dynamic_cast<const MySearcher *>(searcher))->get_memory_part() * MaxMemory)
                (dynamic_cast<MySearcher *>(searcher))->changeAlgorithm();
        }
    }
//    else if (dynamic_cast<const BatchingSearcher*>(searcher) != nullptr) {
//        if (dynamic_cast<NewSearcher*>(dynamic_cast<BatchingSearcher*>(searcher)->baseSearcher) != nullptr) {
//            NewSearcher *n_searcher = dynamic_cast<NewSearcher*>(dynamic_cast<BatchingSearcher*>(searcher)->baseSearcher);

//            if (n_searcher->is_running_BFS()) {
//                unsigned long mbs = (util::GetTotalMallocUsage() >> 20) + (memory->getUsedDeterministicSize() >> 20);

//                double x = n_searcher->get_memory_part();
//                if (mbs >= x * MaxMemory) {
//                    n_searcher->changeAlgorithm();
//                }
//            }
//        }
//    }
...
void Executor::remove_from_states(std::vector<ExecutionState *>states_to_remove)
{
...
void Executor::run(ExecutionState &initialState) {
      mallopt(M_MMAP_MAX, 0);
//    std::cerr << "OVO SE BAR RANUJE\n";

//    std::map<ExecutionState, int>;
  bindModuleConstants();
  ...
  //ODMAH OVDE DODJE, NE ULAZI U IF GORE

  searcher = constructUserSearcher(*this);

//  std::cout << "BROJ STANJA: " << states.size() << "\n";
  std::vector<ExecutionState *> newStates(states.begin(), states.end());
  searcher->update(0, newStates, std::vector<ExecutionState *>());

  while ((!states.empty() && !haltExecution) || (dynamic_cast<MySearcher *>(searcher))->empty() == false) {
//      std::cout << "\nSelect\n";
    ExecutionState &state = searcher->selectState();
//    std::cout << "\nSelected\n";

//    std::cerr << "\n" << state.pc->info->file << " " << state.pc->info->line << " " << state.pc->info->id << "\n";
    KInstruction *ki = state.pc;
//    std::cout << "\nStep\n";
    stepInstruction(state);
//    std::cout << "\nStepped\n";
...
//    if ((dynamic_cast<NewSearcher *>(searcher)) != nullptr) {
//        dynamic_cast<NewSearcher *>(searcher)->Halt();

//        if (dynamic_cast<NewSearcher *>(searcher)->shoud_halt()) {
//            haltExecution = true;
//        }
//    }
...
//    std::cerr << "USAO U Executor::terminateStateOnError\n";
...
//  std::cerr << "specialFunctionHandler->handle\n";
  if (specialFunctionHandler->handle(state, function, target, arguments)) {
//    std::cerr << "specialFunctionHandler->handle hoce ovo\n";
      return;
  }
