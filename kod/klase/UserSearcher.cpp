...
cl::list<Searcher::CoreSearchType> CoreSearch(
    ...
    cl::desc("Specify the search heuristic (default=random-path interleaved "
             "with nurs:covnew)"),
    cl::values(
        clEnumValN(Searcher::MY_SEARCHER, "new_searcher", "using my algorithm"),
  ...
  if (CoreSearch.empty()) {
    if (UseMerge){
      ...
      klee_warning("--use-merge enabled. Using NURS_CovNew as default searcher.");
        ...
        CoreSearch.push_back(Searcher::MY_SEARCHER);
...
bool klee::userSearcherRequiresMD2U() {
  return (std::find(CoreSearch.begin(), CoreSearch.end(), Searcher::NURS_MD2U) != CoreSearch.end() ||
      ...
      std::find(CoreSearch.begin(), CoreSearch.end(), Searcher::MY_SEARCHER) != CoreSearch.end());
}
...
Searcher *getNewSearcher(Searcher::CoreSearchType type, Executor &executor) {
  ...
  case Searcher::MY_SEARCHER: searcher = new MySearcher(MemoryPart, executor); std::cout << "my searcher\n"; break;
  case Searcher::NEW_SEARCHER: searcher = new NewSearcher(MemoryPart, executor); std::cout << "new searcher\n"; break;
  }
