...
namespace klee {
  ...
  class MySearcher : public Searcher {
    std::vector<ExecutionState*> BFSStates;
    std::vector<ExecutionState*> DFSStates;
    bool runningBFS;
    double m_memory_part;
    int instructions;
    Executor &exe;

  public:
    MySearcher(double memory_part, Executor &exec);
    ~MySearcher() {};
    void changeAlgorithm();
    bool is_running_BFS() const;
    ...
    int find_state_index();
    double get_memory_part() const;
